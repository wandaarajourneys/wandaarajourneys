...
async function main() {
  ...
  if (!adminPassword) {
    throw new Error(
      "ADMIN_SEED_PASSWORD is not set. Set it in .env.local before running the seed script.",
    );
  }
  ...
  if (!existingAdmin) {
    ...
    console.log(`Seeded admin account: ${adminEmail} — sign in and change the password immediately.`);
  } else {
