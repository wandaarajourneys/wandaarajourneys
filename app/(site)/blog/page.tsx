...
export default async function BlogPage() {
  ...
  const categories = [
    ...
    "Interactive Packing Lists & Tips",
    "Storytelling & Bush Tales"
  ];
    ...
    <div className="py-16">
      <div className="container-page">
        ...
        {categories.map((category) => {
          const categoryPosts = blogPosts.filter((p) => p.tags.includes(category));
          if (categoryPosts.length === 0) return null;

          return (
            <div key={category} className="mt-16 first:mt-12">
              <Reveal>
                <h2 className="font-display text-2xl text-teal-800 mb-6">{category}</h2>
              </Reveal>
